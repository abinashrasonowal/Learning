...
```
01-SOLID/
  srp/problem/Invoice.java             package srp.problem;
  srp/problem/SRPProblemMain.java      <- driver, not a main inside Invoice
  srp/solution/Invoice.java            package srp.solution;
...
# multi-class package — compile all, then run the driver
javac -Xlint:all -d out $(find . -name '*.java') && java -ea -cp out srp.problem.SRPProblemMain
```

Add `out/` to `.gitignore` if it isn't already.

## Rules for every `solution/` package
...
- state you add must actually be **used**. A field the methods ignore in favour of
  their own parameters is decoration — check by running it and seeing the field's
  value in the output.
- one concept, one type. Don't hold `double amount` and return `int` from the
...
- **DIP** — write a second test double asserting two sends. Impossible in the
  problem package: no seam to inject through.
